...
# Check if CrewChief is already running
if pgrep -f "CrewChiefV4.exe" >/dev/null; then
    echo ""
    echo "⚠️  CrewChief is already running."
...
CC_EXE="$STEAM_DIR/steamapps/compatdata/$game/pfx/drive_c/Program Files (x86)/Britton IT Ltd/CrewChiefV4/CrewChiefV4.exe"
...
###############################################
# Launch CrewChief normally for all games
###############################################
echo "Launching CrewChief..."
export PYTHONWARNINGS="ignore::UserWarning"
protontricks-launch --appid "$game" "$CC_EXE" >/dev/null 2>&1 &
echo "CrewChief has been launched!"

###############################################
# RaceRoom Dash support (AppId 211500)
#
# CHECK IF DASH.EXE IS ALREADY RUNNING, IF NOT, RUN IT. mOVE THIS TO SHARED FUNCTIONS
#Will implement this later, same function is already in the SimHUb LAUNCHER
if [[ "$game" = "21150000000" ]]; then
